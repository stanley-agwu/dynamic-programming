...
const romanToInteger = (roman) => {
  ...
  for(let i = 0; i < romanArray.length; i += 1) {
    if (romanNumeralsMap.get(romanArray[i + 1]) && romanNumeralsMap.get(romanArray[i + 1]) > romanNumeralsMap.get(romanArray[i])) {
      numResult -= romanNumeralsMap.get(romanArray[i])
    } else {
      numResult += romanNumeralsMap.get(romanArray[i])
    }
  }
